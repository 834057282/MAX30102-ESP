 ...
 * @LastEditTime: 2025-06-13 20:37:46
 * @FilePath: \test_s3_max30102\components\max30102\src\max30102.c
 * @Description:
 */
...
{
	return max30102_write(sensor, 0x09, 0X40);
}
...
esp_err_t max30102_config(max30102_handle_t sensor)
{
	...
	return write_sequence(sensor, ((const uint8_t *)REG), ((const uint8_t *)data), sizeof(REG));
}
...
esp_err_t max30102_read_temp(max30102_handle_t sensor, float *temperature)
{
	...
	// 触发温度测量
	ret = max30102_write(sensor, 0x21, 0x01);
	if (ret != ESP_OK)
	{
		return ret;
	}
